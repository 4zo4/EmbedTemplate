/**
 * @file pci.c
 * @brief PCI (Peripheral Component Interconnect)-specific implementations for QEMU x86 Virt.
 * This file contains the implementations of the PCI-specific functions for the QEMU x86 Virt,
...
{
    LOG_PCI_DEBUG("PCI MSI interrupt %u received", MSI_IRQ_VECTOR_BASE + index);
...
void pcie_msi_vector_handler(uint32_t index)
{
    LOG_PCI_DEBUG("PCI MSI interrupt %u received, event 0x%x set", MSI_IRQ_VECTOR_BASE + index, EVT_MSI_IDX(index));
...
int pcie_register_msi_vector(pci_device_info_t *dev_info, uint32_t *vector_idx)
{
    ...
    uint32_t vector_id = MSI_IRQ_VECTOR_BASE + vector_count;
    uint32_t irq_id = vector_id - X86_IRQ_VECTOR_BASE;

#ifdef ENABLE_RTOS
    xPortRegisterCInterruptHandler(pci_trampoline_table[vector_count], vector_id);
    *vector_idx = vector_id;
#else
    apic_register_interrupt(irq_id, pci_trampoline_table[vector_count]);
    *vector_idx = vector_id;
#endif
    apic_enable_interrupt(irq_id);
