...
def remove_qemu_distro(repo_path):
    ...
    if os.path.exists(install_log):
        try:
            ...
            print("[⚠️] Warning: QEMU ninja uninstaller failed")
...
def verify_project_symlinks(project_root_dir, tools_dir, bsp_dir, os_dir, selected_ids):
    ...
    links_to_verify = [
        {
            "target": os.path.join(bsp_dir, "tf-a"),
            "symlink": os.path.join(project_root_dir, "third_party", "bsp", "tf-a"),
            "label": "tf-a -> third_party/bsp/tf-a"
        },
        {
            "target": os.path.join(os_dir, "freertos"),
            "symlink": os.path.join(project_root_dir, "third_party", "os", "freertos"),
            "label": "freertos -> third_party/os/freertos"
        }
    ]
    if "peakrdl" in selected_ids:
        links_to_verify.append({
            "target": os.path.join(tools_dir, "peakrdl"),
            "symlink": os.path.join(project_root_dir, "tools", "peakrdl"),
            "label": "peakrdl -> tools/peakrdl"
        })
...
def align_repo_state(repo_path, tag = None, branch = None):
...
def main():
    ...
    default_bsp_dir = path_defaults.get("clone_bsp_dir", "~/bsp")
